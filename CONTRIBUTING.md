...
3. **Choose a representative path.** Name one real behavior, its first source
   file, supporting implementation and tests, and an entry-to-result trace.
...
6. **Score before consulting gaps.** Record recurring signals and reasons for
   ...
   slot.
7. **Rank same-slot candidates.** At most two repositories may occupy one
   language/Level slot. Keep the two clearest learning paths and record other
   qualified candidates as capacity alternates.
8. **Record the decision.** Accepted entries go in
   ...
   condition.
9. **Get an independent review.** The reviewer checks both gates, the selected
   path, each anchor, formula, capacity result, language, pin, and license.
...
[`catalog/schema.json`](catalog/schema.json) defines schema version 2 and
[`scripts/catalog.py`](scripts/catalog.py) enforces semantic relationships:
safe paths, inspection membership, pinned license URL arrays, formula, order,
uniqueness, capacity, and rebuild-audit reconciliation.
...
- Goal, start reason, supporting files, trace, prerequisites, and domain context
  describe the path actually scored.
