...
- `catalog/<language>.json` contains accepted schema-version-2 entries.
...
- `research/audit.json` is historical until the final learner-centered corpus
  audit replaces it.
- `docs/learning-levels.md`, `docs/qualification.md`, and
  `docs/research-process.md` define human review.
...
product is reviewed canonical data and generated Markdown.
...
has two qualified repositories at every Level. Commit canonical input and
generated output together.

## Refreshing an accepted entry

1. Establish the exact catalog file, repository, pin, learning path, four
   scores, quality findings, inspection files, language evidence, and license
   URLs.
...
7. Update the commit, date, reviewers, inspected files, path, domain context,
   learner prose, scores, metadata, and license URLs as one evidence unit.
...
4. research up to two qualified entries per Level without lowered standards;
...
- public repository, archive state, language classification, and unique use;
- exact pin and every safe learning-path and inspection path;
- coding-relevance and all eight quality findings;
- four scores, formula, capacity, prerequisites, and domain context;
...
set, rejected set, and remaining gaps in the current audit. Run the final gate
from an isolated committed snapshot that explicitly excludes all dotenv naming
variants.
